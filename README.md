...
Company Decision Monitor 是面向普通用户的公司研究与企业动态监控 Windows 桌面软件。v0.1.2 不要求用户导入 Excel、CSV 或本地公司数据库；用户可以通过公开数据源和自行申请的免费 API key 搜索公司、查看真实 provider 返回的基础资料、新闻和自选刷新状态。
