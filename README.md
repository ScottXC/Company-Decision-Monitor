...
Current version: `v0.1.2-core-functions`
...
## v0.1.2-core-functions

This version focuses on making the Public + Free API Network Mode loop actually work end to end:
...
当前版本：`v0.1.2-core-functions`  
当前模式：`Public + Free API Network Mode`

Company Decision Monitor 是面向普通用户的公司研究与企业动态监控 Windows 桌面软件。v0.1.2-core-functions 不要求用户导入 Excel、CSV 或本地公司数据库；用户可以通过公开数据源和自行申请的免费 API key 搜索公司、查看真实 provider 返回的基础资料、新闻和自选刷新状态。
...
不同国家注册数据源的认证、字段、限流和服务条款差异较大。v0.1.2-core-functions 只接入已完成基础映射的来源，其他来源保留为 registry/stub，以免展示不稳定或不可复现的数据。
